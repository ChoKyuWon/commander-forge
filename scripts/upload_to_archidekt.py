...
def update_deck(token, deck_path, resolved, dry_run=False):
...
def main():
    ...
    ap.add_argument("--private", action="store_true", help="make the deck private")
    ap.add_argument("--unlisted", action="store_true", help="make the deck unlisted")
    ...
    log("creating deck...")
    deck_id = create_deck(token, name, deck_format, args.bracket, args.private, args.unlisted)
    if not deck_id:
        return 5
